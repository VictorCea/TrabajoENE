...
import { EntityArrayResponseType, ProveedorService } from '../service/proveedor.service';
...
export class ProveedorComponent implements OnInit {
  ...
  trackId = (item: IProveedor): number => this.proveedorService.getProveedorIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.proveedors().length === 0) {
            ...
            this.proveedors.set(this.refineData(this.proveedors()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.proveedors.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IProveedor[]): IProveedor[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IProveedor[] | null): IProveedor[] {
    return data ?? [];
  }
